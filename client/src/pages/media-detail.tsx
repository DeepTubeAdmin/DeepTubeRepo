...
export default function MediaDetail() {
  ...
  const handleSocialShare = (platform: string) => {
    const shareUrl = `${window.location.origin}/media/${id}`;
    const shareTitle = media
      ? media.title
      : "Check out this content on DeepTube";
    const shareText =
    ...
    switch (platform) {
      ...
      case "tiktok":
        ...
        toast({
          ...
          description:
            "Copy the embed code from above first, then use it in your TikTok post.",
        });
        break;
      case "instagram":
        ...
        toast({
          ...
          description:
            "Copy the link or embed code first, then use it in your Instagram post.",
        });
        break;
      default:
        return;
            ...
            <div className="bg-[#121212] rounded-md overflow-hidden mb-4">
              {media.contentType === "video" && (
                <div className="aspect-video">
                  ...
                  <GenericVideoEmbed
                    videoUrl={
                      media.videoUrl || `/api/videos/${media.id}/direct`
                    }
                    title={media.title}
                    responsive={true}
                    autoplay={true}
                ...
                <div className="flex items-center justify-center bg-black relative group">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      if (
                        media.imageUrl &&
                        media.imageUrl.includes("/home/runner/workspace/")
                      ) {
                        const match = media.imageUrl.match(
                          /\/home\/runner\/workspace\/(.+)/,
                        );
                        if (match && match[1]) {
                          cleanedUrl = `/${match[1]}`;
                          console.log(
                            `Image: Cleaned workspace path: ${media.imageUrl} → ${cleanedUrl}`,
                          );
                      ...
                      if (
                        media.imageUrl &&
                        media.imageUrl.startsWith("/api/s3/home/")
                      ) {
                        ...
                        if (pathParts.length > 1) {
                          ...
                          console.log(
                            `Image: Converted absolute path: ${media.imageUrl} → ${cleanedUrl}`,
                          );
                          e.currentTarget.src = cleanedUrl;
                          return;
                        }
                          ...
                          __html: media.embedCode.includes("autoplay=1")
                            ? media.embedCode.includes("origin=")
                              ...
                              : media.embedCode
                                  .replace(
                                    /src="([^"]+)"/,
                                    `src="$1&origin=${window.location.origin}"`,
                                  )
                                  .replace(/width="\d+"/, 'width="100%"')
                                  .replace(/height="\d+"/, 'height="100%"')
                            : media.embedCode
                                .replace(
                                  /src="([^"]+)"/,
                                  `src="$1?autoplay=1&enablejsapi=1&origin=${window.location.origin}"`,
                                )
                                .replace(
                                  /src="([^"]+)\?([^"]*)"/,
                                  `src="$1?autoplay=1&enablejsapi=1&origin=${window.location.origin}&$2"`,
                                )
                                .replace(/width="\d+"/, 'width="100%"')
                                .replace(/height="\d+"/, 'height="100%"'),
                        }}
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-3">
                  ...
                  >
                    <ThumbsUp
                      className={`w-5 h-5 mr-1 ${isLikeLoading ? "animate-pulse" : ""}`}
                    />
              ...
              <div className="mt-6 mb-8 space-y-4">
                ...
                <div className="bg-[#0c0c0c] p-4 rounded-md border border-[#222]">
                  ...
                  <p className="text-gray-300 text-sm">
                    {media.prompt || "No prompt information provided."}
                  </p>
                </div>
                    ...
                    <div key={comment.id} className="flex gap-3">
                      <Avatar className="w-10 h-10">
                        <AvatarImage
                          src={
                            comment.userId
                              ? `/api/users/${comment.userId}/avatar`
                              : undefined
                          }
                        />
                        <AvatarFallback>
          ...
          <div className="py-4">
            ...
            <div className="mt-6 mb-6">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Copy this code and paste it into your post to embed this content
                on other platforms.
              </p>
            </div>
