...
export default function ImageCard({ image, size = 'default' }: ImageCardProps) {
  ...
  const formatAIGenerator = (generator: string): string => {
    ...
    return generator.split(/[ -_]+/).map(part => {
      ...
      >
        <Link href={`/media/${image.id}`}>
          <div className={`${aspectRatioClass} bg-slate-800 relative overflow-hidden`}>
            ...
            {/* Overlay */}
            <div 
              className={`absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/40 to-transparent transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-80'}`} 
            />
