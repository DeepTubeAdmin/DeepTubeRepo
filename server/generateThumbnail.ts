...
export async function generateAndStoreS3Thumbnail(
  ...
  if (contentType === "video" && sourceUrl) {
    try {
      ...
      const videoPath = actualVideoPath.startsWith("/")
        ? actualVideoPath.substring(1)
        : actualVideoPath;
      ...
      try {
        ...
        console.error(
          "Failed to generate thumbnail directly from file:",
          error,
        );
      }
      ...
      while (attempts < 3) {
        try {
          ...
          if (attempts === 3)
            throw new Error(
              `Failed to download video after 3 attempts: ${err}`,
            );
      ...
      for (const ts of timestamps) {
        ...
        try {
          console.log(
            `Attempting thumbnail extraction at ${ts}s with standard method`,
          );
          await execFileAsync("ffmpeg", [
            "-y",
            "-i",
          ...
          success = true;
          console.log(
            `Successfully extracted frame at ${ts}s with standard method`,
          );
        } catch (err) {
        ...
        if (!success) {
          try {
            console.log(
              `Attempting thumbnail extraction at ${ts}s with seek-before-input method`,
            );
            await execFileAsync("ffmpeg", [
              "-y",
              "-ss",
            ...
            success = true;
            console.log(
              `Successfully extracted frame at ${ts}s with seek-before-input method`,
            );
          } catch (err) {
        ...
        if (!success) {
          try {
            console.log(
              `Attempting thumbnail extraction at ${ts}s with forced decoder method`,
            );
            await execFileAsync("ffmpeg", [
              "-y",
              "-c:v",
            ...
            success = true;
            console.log(
              `Successfully extracted frame at ${ts}s with forced decoder method`,
            );
          } catch (err) {
      ...
      if (!success) {
        throw new Error(
          "Failed to extract video frame after multiple attempts",
        );
      }
      ...
      console.log(
        `Successfully generated thumbnail of size ${thumbBuffer.length} bytes`,
      );
  ...
  throw new Error(
    "Failed to extract video frame for thumbnail - no fallback to SVG",
  );
}
