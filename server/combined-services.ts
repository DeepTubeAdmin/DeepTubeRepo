...
/**
 * Reads a file from disk, uploads it to S3 and **always** removes the
 * temporary file afterwards.
 *
 * @param filePath local temp path (produced by multer)
 * @param s3Key    the bucket key where the object should be stored
 * @returns        the signed/public S3 URL
 */
export async function uploadFileToS3(
  ...
  try {
    ...
    // push to S3
    // -----------------------------------------------------------------------
    const url = await s3Service.uploadToS3(fileBuffer, s3Key, { contentType });
    console.log(`Uploaded to S3 (${s3Key}) → ${url}`);
    return url;
  } catch (err) {
    ...
    // -----------------------------------------------------------------------
    // ALWAYS delete the temp file – even if the upload failed
    // -----------------------------------------------------------------------
...
export async function generateAndStoreS3Thumbnail(
  ...
  console.log(
    `Thumbnail generation request for ${contentType} ${contentId} using FFmpeg`
  );
  try {
