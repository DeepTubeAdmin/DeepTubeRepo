...
export interface IStorage {
  ...
  getApprovedContent(
    page?: number,
    limit?: number
  ): Promise<{ content: Video[]; totalCount: number }>;
  ...
  updateForumComment(
    id: number,
    data: Partial<InsertForumComment>
...
export class DatabaseStorage implements IStorage {
  ...
  async getVideos(
    ...
    console.log(
      `getVideos called with contentType=${
  ...
  async getVideosByCategory(
    ...
    if (contentType) {
      console.log(
        `Filtering by category=${categoryId} and contentType="${contentType}"`
      );
  ...
  async deleteVideo(id: number): Promise<void> {
    ...
    try {
      ...
      await db.delete(wishlistItems).where(eq(wishlistItems.videoId, id));
  ...
  // Content review operations
  async getPendingReviewContent(
    limit: number = 50
  ): Promise<(Video & { uploaderName?: string })[]> {
  ...
  async searchVideos(options: {
    ...
    const fullQueryEscaped = preprocessedQuery
      .replace(/%/g, "\\%")
      .replace(/_/g, "\\_");
    searchConditions.push(
      or(
    ...
    const relevanceSortSql = sql`CASE WHEN ${
      videos.title
    } ILIKE ${`%${fullQueryEscaped}%`} THEN 1 ELSE 0 END DESC`;
    ...
    try {
      ...
      // Log search results
      console.log(
        `Search for "${query}" found ${results.length} results after preprocessing`
      );
  ...
  async removeLike(
    ...
    try {
      ...
      if (userId) {
        ...
        throw new Error(
          "Must provide either userId or both ipAddress and sessionId"
        );
      }
  ...
  async incrementViews(videoId: number): Promise<number> {
    try {
      ...
      console.log(
        `Incremented views for video ${videoId} to ${updatedVideo.views}`
      );
  ...
  async getMostViewedVideos(
    ...
    try {
      ...
      console.log(
        `Retrieved ${results.length} most viewed videos with category ${
          categoryId || "none"
        }. First few IDs:`,
  ...
  async getContentByType(
    ...
    try {
      ...
      // Visibility: approved or youtube embed
      conditions.push(
        or(
          eq(videos.reviewStatus, "approved"),
          and(
      ...
      const useShuffle = shuffleSeed && !isChronological;

      // If shuffle is requested and valid
      if (useShuffle) {
        const seedValue =
        ...
        switch (sortBy) {
          ...
          case "popular":
            query = query.orderBy(sql`
                      (SELECT COUNT(*) FROM ${likes} WHERE ${likes.videoId} = ${videos.id}) DESC
                    `);
            break;
          case "trending":
          default:
      ...
      console.log("RAW SQL QUERY:", rawSql.sql, rawSql.params);
  ...
  async blockUser(userId: number, blockedUserId: number): Promise<BlockedUser> {
    try {
      ...
      console.error(
        `Error blocking user ${blockedUserId} by user ${userId}:`,
        error
      );
      throw error;
    }
  ...
  async createMessage(message: InsertMessage): Promise<Message> {
    ...
    try {
      ...
      console.log(
        "DatabaseStorage: Message created successfully with ID:",
        result.id
      );
      return result;
    } catch (error) {
  ...
  async deleteConversation(userId: number, otherUserId: number): Promise<void> {
    try {
      ...
      console.log(
        `Deleted conversation between user ${userId} and user ${otherUserId}`
      );
    } catch (error) {
      console.error(
        `Error deleting conversation between user ${userId} and user ${otherUserId}:`,
        error
      );
      throw error;
    }
  ...
  async createForumThread(thread: InsertForumThread): Promise<ForumThread> {
    ...
    const user = newThread.userId
      ? await this.getUser(newThread.userId)
      : undefined;
  ...
  async updateForumThread(
    ...
    const user = updatedThread.userId
      ? await this.getUser(updatedThread.userId)
      : undefined;
  ...
  async createForumComment(comment: InsertForumComment): Promise<ForumComment> {
    ...
    const user = newComment.userId
      ? await this.getUser(newComment.userId)
      : undefined;
  ...
  async updateForumComment(
    ...
    // Fetch the user
    const user = updatedComment.userId
      ? await this.getUser(updatedComment.userId)
      : undefined;
